...
use queries_for_sqlx::{
    delete_st, insert_many_st::insert_many, select_st,
};
...
pub struct ManyToManyDynamic<From, To> {
    pub list_itself_under: String,
    pub key: String,
    pub rel_spec: ManyToMany,
    pub _pd: PhantomData<(From, To)>,
}
...
where
    F: Collection + 'static,
    T: Collection + 'static + Serialize + DeserializeOwned,
{
        ...
        async move {
            ...
            st.select(
                ft(self.rel_spec.conjuction_table.clone())
                    .col(self.rel_spec.destination_id.clone())
                    .alias("dest_id".to_string()),
            );

            Destination::on_select1(&mut st);

            st.where_(
                col(self.rel_spec.base_id.clone())
                    .eq(move || id),
            );

            st.join(Join {
                ty: "LEFT JOIN",
                on_table: Destination::table_name1().to_owned(),
            ...
            let vals = st
                .fetch_all(&pool, |row| {
                    let val =
                        Destination::from_row_noscope2(&row);
    ...
    T: Collection,
{
    ...
    fn sub_op2<'this>(
        ...
        async move {
            ...
            // self.input
            let mut st =
                SelectSt::init(T::table_name1().to_string());

            st.where_(or(self
                .input
                .iter()
                .cloned()
                .map(|e| col("id".to_string()).eq(move || e))
                .collect()));
    ...
    T: Collection,
{
    ...
    fn sub_op<'this>(
        ...
        async move {
            ...
            st.select(
                col(self.spec.base_id.clone())
                    .alias("from_id".to_string()),
            );

            st.select(
                col(self.spec.destination_id.clone())
                    .alias("dest_id".to_string()),
            );
            ...
            st.where_(or(id_set
                ...
                .map(|id| {
                    let id = id.clone();
                    col(self.spec.base_id.clone()).eq(move || id)
                })
                .collect()));

            st.join(Join {
                ty: "LEFT JOIN",
                on_table: T::table_name1().to_owned(),
                on_column: "id".to_string(),
